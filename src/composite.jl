...
@eval function Base.exp(t::Phasor{V,<:TensorGraded},$(args...)) where V
    z = exp(angle(t),$(args...))
    Phasor{V}(exp(amplitude(t)+realvalue(z)),imagvalue(z))
end
...
@eval begin
    ...
    $logm(t::Phasor,$(args...)) = (r=amplitude(t); $logm(r,$(args...))+angle(t))
...
for (qrt,n) ∈ ((:sqrt,2),(:cbrt,3))
    @eval begin
        ...
        @inline Base.$qrt(t::Phasor,$(args...)) = Phasor($qrt(amplitude(t),$(args...)),angle(t)/$n)
