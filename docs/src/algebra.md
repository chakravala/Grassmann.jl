...
The mixed index algebra ``\Lambda(T^\mu V) = (\bigoplus_{g=1}^n\Lambda^g V)\oplus(\bigoplus_{g=1}^\mu L^g V)`` is partitioned into both symmetric and anti-symmetric tensor equivalence classes. Any mixed tensor `Basis` pair ``\omega,\eta`` satisfies either
```math
...
While `Grassmann.Algebra{V}` is a container for the `TensorAlgebra` generators of ``V``, the `Grassmann.Algebra` is only cached for ``N\leq8``.
For the range of dimensions ``8<N\leq22``, the `Grassmann.SparseAlgebra` type is used.
```julia
julia> Λ(22)
...
```
Multiplication with an ``\epsilon_i`` element is used help signify tensor fields so that differential operators are automatically applied in the `Basis` algebra as ∂ⱼ⊖(ω⊗ϵᵢ) = ∂ⱼ(ωϵᵢ) ≠ (∂ⱼ⊗ω)⊖ϵᵢ.
```julia
